...
public class EmployeeImpl implements EmployeeDao {
	...
	private Employee getEmployee(ResultSet rs) throws SQLException {
		...
		Department dept = new Department(rs.getInt("deptNo"));
			...
			try {
				manager.seteName(rs.getString("manager_name"));
			} catch (SQLException e) {}
				
					try {
				dept.setdName(rs.getString("deptName"));	
			} catch (SQLException e) {}
	...
	public int insertEmployee(Employee employee) {
		...
		try (PreparedStatement pstmt = con.prepareStatement(sql)) {
			...
			pstmt.setInt(4, employee.getManager().geteNo());
			pstmt.setInt(5, employee.getSalary());
			pstmt.setInt(6, employee.getDept().getdNo());
	...
	public int updateEmployee(Employee employee) {
		...
		try (PreparedStatement pstmt = con.prepareStatement(sql)) {
			...
			pstmt.setInt(3, employee.getManager().geteNo());
			pstmt.setInt(4, employee.getSalary());
			pstmt.setInt(5, employee.getDept().getdNo());
			pstmt.setInt(6, employee.geteNo());
			return pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		}
